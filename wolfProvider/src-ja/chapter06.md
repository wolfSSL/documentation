...
wolfProvider は以下のロギング レベルをサポートします。 これらは、"include/wolfprovider/we_logging.h"ヘッダー ファイルで、wolfProvider_LogType enum の一部として定義されています:
...
デフォルトの wolfProvider ロギング レベルには、"WE_LOG_ERROR"、"WE_LOG_ENTER"、"WE_LOG_LEAVE"、および"WE_LOG_INFO"が含まれます。 これには、詳細ログ (`WE_LOG_VERBOSE`) を除くすべてのログ レベルが含まれます。
...
wolfProvider では、コンポーネントごとにログを記録できます。 コンポーネントは `include/wolfprovider/we_logging.h` の wolfProvider_LogComponents 列挙で定義されます:
...
| WE_LOG_PROVIDER | エンジン特有 | 0x0040 |
| WE_LOG_COMPONENTS_ALL | 全コンポーネント | WE_LOG_RNG &#124; WE_LOG_DIGEST &#124; WE_LOG_MAC &#124; WE_LOG_CIPHER &#124; WE_LOG_PK &#124; WE_LOG_KE &#124; WE_LOG_PROVIDER |
| WE_LOG_COMPONENTS_DEFAULT | デフォルトコンポーネント (all). | WE_LOG_COMPONENTS_ALL |


デフォルトの wolfProvider ロギング構成は、すべてのコンポーネントをログに記録します (`WE_LOG_COMPONENTS_DEFAULT`)。
...
ログ メッセージの出力方法や出力場所をより詳細に制御したいアプリケーションは、カスタム ロギング コールバックを記述して wolfProvider に登録できます。 ロギング コールバックは、`include/wolfprovider/we_logging.h` の wolfProvider_Logging_cb のプロトタイプと一致する必要があります:
